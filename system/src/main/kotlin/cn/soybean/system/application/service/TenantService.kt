...
class TenantService(private val tenantQueryService: TenantQueryService, private val commandInvoker: CommandInvoker) {
    fun createTenant(command: CreateTenantCommand): Uni<Pair<Boolean, String>> = checkTenantName(command.name).flatMap { (flag, msg) ->
        when {
    ...
    fun updateTenant(command: UpdateTenantCommand): Uni<Pair<Boolean, String>> = when {
        command.id.isBlank() -> Uni.createFrom().item(Pair(false, "ID cannot be null or blank."))
            ...
            checkTenantNameForUpdate(command.name, command.id).flatMap { (flag, msg) ->
                when {
                    flag ->
    ...
    fun deleteTenant(command: DeleteTenantCommand): Uni<Pair<Boolean, String>> = Multi.createFrom().iterable(command.ids)
    ...
    private fun checkTenantName(name: String): Uni<Pair<Boolean, String>> = when (name) {
        DbConstants.SUPER_TENANT ->
            ...
            tenantQueryService.handle(TenantByNameExistsQuery(name))
                .flatMap { exist ->
                    when {
    ...
    private fun checkTenantNameForUpdate(name: String, id: String): Uni<Pair<Boolean, String>> = when (name) {
        DbConstants.SUPER_TENANT ->
            ...
            tenantQueryService.handle(TenantByIdQuery(id))
                .flatMap { tenant ->
                    when {
