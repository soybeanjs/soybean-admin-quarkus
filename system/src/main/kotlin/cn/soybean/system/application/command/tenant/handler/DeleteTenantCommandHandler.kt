...
@ApplicationScoped
class DeleteTenantCommandHandler(
    private val eventStoreDB: EventStoreDB,
    private val loginHelper: LoginHelper,
) : CommandHandler<DeleteTenantCommand, Boolean> {
    override fun handle(command: DeleteTenantCommand): Uni<Boolean> =
        Multi
            .createFrom()
            ...
            .transformToUniAndMerge { id ->
                eventStoreDB
                    .load(id, TenantAggregate::class.java)
                    .map { aggregate ->
            ...
            .replaceWith(true)
            .onFailure()
            .invoke { ex -> Log.errorf(ex, "DeleteTenantCommandHandler fail") }
